...
public class LoginController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        User u = db.getAccount(email, password);
        //User user = db.checkEmail(email);
        if (u == null) {
//            response.getWriter().println("username/password is invalid!");
//            response.sendRedirect("Login");
            request.setAttribute("msg", "Email/Password is invalid!"
                    + "\n" + " Please check your information and relogin!");
            ...
            response.getWriter().println("Ok" + email);
            UserDAO udao = new UserDAO();
            RoleList roles = udao.listRole(u.getId());
            session.setAttribute("roles", roles);
            if (roles.isAdmin()) {
                response.sendRedirect("manage/dashboard/view");
            } else {
