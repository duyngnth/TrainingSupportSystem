...
public class WebContactDetailController extends HttpServlet {
    ...
    throws ServletException, IOException {
        int raw_id = Integer.parseInt(request.getParameter("id"));
        
        WebContactDBContext wdc = new WebContactDBContext();
        WebContact wc = wdc.get(raw_id);
        
        request.setAttribute("wc", wc);
        request.getRequestDispatcher("../view/WebContactDetail.jsp").forward(request, response);
    } 
    ...
    throws ServletException, IOException {
        ...
        String res = request.getParameter("response").trim();
        if (res.length() == 0) {
            res = null;
        }
